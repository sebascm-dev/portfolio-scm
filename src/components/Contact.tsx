...
export default function Contact({ }: Props) {
    ...
    const sendEmail = (e: FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        setSending(true); // Comienza la animación de carga
            ...
            emailjs
                ...
                .then(
                    (result) => {
                        ...
                        setSending(false); // Finaliza la animación de carga
                        setMessageSent(true); // Establece el mensaje de enviado
                        setTimeout(() => {
                            setMessageSent(false); // Después de un tiempo, vuelve al estado original
                        }, 3000); // 3000ms (3 segundos) antes de volver al estado original
                    },
                    (error) => {
                        console.log(error.text);
                        setSending(false); // Finaliza la animación de carga si hay un error
                    }
                );
        }
    };
            ...
            <div className="flex flex-col sm:flex-row items-stretch justify-between gap-5">
                <form ref={form} onSubmit={sendEmail} className="p-10 h-[40em] mb-8 bg-[#373737] rounded-md shadow-md w-full sm:w-1/3 flex-grow">
                    ...
                    <input
                        ...
                        className={`mail-btn w-full px-4 py-2 mt-2 mb-4 font-bold text-white ${messageSent ? 'bg-green-600' : 'bg-[#F5B041]'} hover:bg-yellow-700 cursor-pointer transition-all duration-500 rounded-md focus:outline-none focus:shadow-outline`}
